...
void get_firmware(char buffer[], char msg[]){
...
static void task_process(void *pvParameters) {
  ...
  while(1){
    ...
    switch(state){
      ...
      case GET:
      printf("STATE: GET \n");
	  get_firmware(g_sendBuffer, MAIN_PREFIX_BUFFER);
      ...
      if(xQueueReceive(xQueueMsg, &p_recvMsg, 5000) == pdTRUE){
        printf(STRING_LINE);
        printf(p_recvMsg->pu8Buffer);
		status = strstr(p_recvMsg->pu8Buffer, command);
		printf("response: %s", status);
		printf("status: %c", status[6]);
		if(!strcmp(status[6], '1')){
			
			pio_clear(LED_BOARD_PIO, LED_BOARD_IDX_MASK);
		}
		else if(!strcmp(status[6], '0')){
...
int main(void)
{
  ...
  pio_set_output(LED_BOARD_PIO, LED_BOARD_IDX_MASK, 0, 0, 0);
